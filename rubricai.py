    ...
    try:
        ...
        if mode == "Feedback":
            ...
            prompt = (
                ...
                f" Begin by analyzing the student's assignment to understand its structure and style. Recognize if it follows an essay, report, or paragraph format."
                f" Match your perfected version's layout and structure to the identified format of the student's assignment."
                f" When creating the perfect-scoring version, continuously refer back to the rubric to ensure all criteria are fully addressed and maximized to their highest potential."
                f" Ensure every criterion in the rubric is completely satisfied to achieve the highest possible score."
                f" Your language and tone should closely align with the student's style, ensuring the perfected version feels natural to their work while being polished and professional."
                f" Make sure you are expanding as much as you can, aiming for depth and specificity, for each section of their assignment to ensure a perfect scoring version of their assignment"
                f" Your response must be formatted with HTML for clarity and structure. Follow these requirements:\n"
                f"1. For essays, use:\n"
                f"   - `<h2>` for the title.\n"
                f"   - `<h3>` for sections such as Introduction, Body Paragraphs, and Conclusion.\n"
                f"   - `<p>` for each paragraph.\n"
                f"2. For reports, use:\n"
                f"   - `<h2>` for the report title.\n"
                f"   - `<h3>` for sections such as Abstract, Methods, Results, and Discussion.\n"
                f"   - `<ul>` or `<ol>` for lists, using `<li>` for each list item.\n"
                f"3. For single-paragraph assignments, refine the paragraph using `<p>` tags and enhance clarity and depth while maintaining conciseness.\n"
                f"Ensure the response is professional, well-organized, and polished."
                f" Exclude the student's original assignment and the rubric from your response."
                f" Provide only the perfected version of the assignment in valid HTML format."
                f" Ensure the output is formatted cleanly as valid HTML without any additional markers or code block wrappers."
